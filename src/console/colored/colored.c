...
colored_ansi256_format(colored_context_t context, colored_color_t code) {
  return string_format(CONSOLE_ASCII_ESCAPE "[%d;5;%" SCNu8 "m", context, code);
}
...
colored_truecolor_format(colored_context_t context, const struct colored_rgb *rgb) {
  return string_format(CONSOLE_ASCII_ESCAPE "[%d;2;%" SCNx8 ";%" SCNx8 ";%" SCNx8 "m", context, rgb->red, rgb->green, rgb->blue);
}
...
bool
colored_isansi16(colored_color_t code) {
/*
...
colored_color16_byname(const char *name) {
...
colored_color(const char *string) {
  ...
  if (!(color = colored_color16_byname(string)))
    return colored_reset();
...
colored_color16_bycode(colored_color_t code) {
  return (colored_isansi16(code) ? (struct colored_color *)&colored_colors16[code] : NULL);
}
...
colored_color16(colored_context_t context, colored_color_t code) {
  ...
  if (!(color = colored_color16_bycode(code)))
    return colored_reset();
...
bool
colored_isansi256(colored_color_t color) {
  return (color <= 0xFF);
}
...
colored_color256(colored_context_t context, colored_color256_t code) {
  ...
  if (colored_isansi16(code) && (color = colored_color16_bycode(code)))
    return colored_colorize(context, color);
...
colored_rgb256(const char *string) {
  ...
  return colored_color256(context, colored_rgb_tocolor256(&rgb));
}
...
colored_style_byname(const char *name) {
...
colored_style(const char *string) {
  ...
  if (!(style = colored_style_byname(string)))
    return colored_reset();
